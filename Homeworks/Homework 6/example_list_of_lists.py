...
def in_common1(state1, state2, state3):
    common = list(state1[2] & state2[2] & state3[2])
    common.sort()
    
    if len(common) == 0:
        print("States {} and {} and {} have no companies in common".format(state1[1], state2[1], state3[1]))
    else:
        print("States {} and {} and {} have {} companies in common".format(state1[1], state2[1], state3[1], len(common)))
        print("Here they are in alphabetical order:")
        i = 0
        for i in range(len(common)):
            print('\t{}: {}'.format(i,common[i]))
...
if __name__ == "__main__":
    ...
    for i in range(1,len(all_states)):
        ...
        print()
        in_common1(all_states[0], all_states[1], all_states[i])
